...
function _simg($filename, $dim = '640x480', $cut_enabled=false)
{
    ...
    if(config_item('strict_image_speed_enable')!==FALSE) {
...
function sw_add_inline_script($handle, $js_inline = NULL, $compress_enabled=true, $cache_time_sec = 86400) {
    static $handle_array_js;
    
    if(!isset($handle_array_js)) $handle_array_js = array();
    
    if(!isset($handle_array_js[$handle])) $handle_array_js[$handle] = '';
    ...
    $handle_cache = FCPATH.'templates/'.$template_name.'/assets/cache/'.$handle.'.js';
    if($original_file === NULL) {
        if(file_exists($handle_cache) && filemtime($handle_cache) > time()-$cache_time_sec)
        {
        } else {
            file_put_contents(FCPATH.'templates/'.$template_name.'/assets/cache/'.$handle.'.js', $handle_array_js[$handle]);
        }
