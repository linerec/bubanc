    ...
    /* End DataTable ini */
     ...
     reloadElements();
        ...
        function manualSearch(v_pagenum, scroll_enabled, onlysave, only_update)
        {
            ...
            $('.sw-search-start').find('.fa-ajax-indicator').removeClass('hidden');
            ...
            <?php if($page_id!=get_results_page_id()): ?>
                if(!$('#results_conteiner').length){
                    ...
                    if(!get_results_page_id()) {
                        echo 'ShowStatus.show("'.lang_check("Please define results page in settings").'");';
                        echo 'return false;';
                    }
                    ...
                    if($value->field_id==$tree_field_id){
                        ...
                        if(!empty($value->image_filename))
                        {
                            ...
                            if($files_r and isset($files_r[1]) and file_exists(FCPATH.'files/thumbnail/'.$files_r[1]->filename)) {
                                ?>
                                $tree_field_icons['<?php _jse($value->value_path);?>']= '<?php echo base_url('files/'.$files_r[1]->filename);?>';
                                <?php
                            }
                        }
            ...
            $('.sw-search-start').find('.fa-ajax-indicator').addClass('hidden');
        });
            return false;
        }
