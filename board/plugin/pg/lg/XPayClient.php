...
class XPayClient
{
	...
	public function __construct($home_dir, $mode="real")
	{
        ...
        //로그파일을 만들지 않습니다. 로그 파일을 만들경우 아래 주석을 풉니다.
...
//		$this->log("session id = ".session_id().$_SESSION['tx_counter'], LGD_LOG_FATAL);
	...
	{
		if ($this->config['output_UTF8'] == 1)
			return isset($this->response_array["LGD_RESPONSE"][$index][$name]) ? $this->response_array["LGD_RESPONSE"][$index][$name] : '';
		else
			return isset($this->response_array["LGD_RESPONSE"][$index][$name]) ? (iconv("utf-8", "euc-kr", $this->response_array["LGD_RESPONSE"][$index][$name])) : '';
	}
	...
	function Log($msg, $level=LGD_LOG_FATAL)
	{
        ...
        //로그를 남기지 않도록 수정했습니다. ( 로그를 남기려면 아래 주석을 풀것 )
